...
- Persistence uses NiceGUI storage. By default, `app.storage.user` is stored
  server-side in the `.nicegui` folder (or Redis if configured).
...
def now_ny_string() -> str:
    """Best-effort date string (month day, year)."""
...
def labeled_input(label: str, *, value: str = "", placeholder: str = "", classes: str = "", props: str = "") -> ui.input:
    return ui.input(label=label, value=value, placeholder=placeholder).props(f"outlined dense stack-label {props}".strip()).classes(classes)


def labeled_select(label: str, options: Any, *, value: Any = None, classes: str = "", props: str = "") -> ui.select:
    return ui.select(options=options, label=label, value=value).props(f"outlined dense stack-label {props}".strip()).classes(classes)


def labeled_textarea(label: str, *, value: str = "", placeholder: str = "", classes: str = "", props: str = "") -> ui.textarea:
    return ui.textarea(label=label, value=value, placeholder=placeholder).props(f"outlined dense stack-label autogrow {props}".strip()).classes(classes)


def cell_is_textarea(section_id: str, field_key: str) -> bool:
    if section_id in {"env", "perm", "zoning", "existing", "zlu"}:
        return True
    ...
    with ui.tab_panels(tabs, value=tab1).classes("w-full"):
                    ...
                    with ui.column().classes("col-6"):
                        with ui.card().classes("section-card w-full"):
                            ...
                            async def do_lookup() -> None:
                                ...
                                for k, label in [
                                    ("owner", "Owner / Parcel Name"),
                                    ("city", "City / Jurisdiction"),
                                    ("address", "Site Address"),
                                    ("acreage", "Site Acreage (gross)"),
        ...
        with ui.tab_panel(tab3):
            with ui.card().classes("w-full tab-card"):
                ...
                ui.label("NiceGUI user storage is saved server-side in the `.nicegui` folder (unless you configure Redis).").classes("muted")
